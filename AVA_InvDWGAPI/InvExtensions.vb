...
Public Module InvExtensions
    ...
    <Extension> Public Function findCurvesByAttribute(aView As Inventor.DrawingView, AttributeSetName As String,
        ...
        If aView.ReferencedDocumentDescriptor.ReferencedDocumentType = DocumentTypeEnum.kAssemblyDocumentObject Then
            ...
            Next
        ElseIf aView.ReferencedDocumentDescriptor.ReferencedDocumentType = DocumentTypeEnum.kPartDocumentObject Then

            Dim oCollection As Inventor.ObjectCollection = CType(aView.ReferencedDocumentDescriptor.ReferencedDocument,
                 Inventor.PartDocument).AttributeManager.FindObjects(AttributeSetName,
                                                                     AttributeName,
                                                                     AttributeValue)
            ...
            For Each oCObject In oCollection
                ...
                If CType(oCObject, Inventor.Face).Type = ObjectTypeEnum.kFaceObject Then
                    Dim oFace = CType(oCObject, Inventor.Face)
                    'Hand over the face, since it may be a face that can also give us silhouette edges...
                    For Each candidateCurve As DrawingCurve In aView.DrawingCurves(oFace)
                        returnCurves.Add(candidateCurve)
                    Next
                End If
            Next
...
#If DEBUG Then
        Dim myDoc As DrawingDocument = CType(aDrawingCurve.Parent.Parent.Parent, DrawingDocument)


        'Dim oTG As TransientGeometry
        'oTG = CType(aDrawingCurve.Parent.Parent.Application, Inventor.Application).TransientGeometry

        ' Get the transaction manager from the application
        Dim oTxnMgr As TransactionManager = CType(aDrawingCurve.Parent.Parent.Application, Application).TransactionManager

        ' Start a regular transaction
        Dim oTxn1 As Transaction = oTxnMgr.StartTransaction(myDoc, "Debug Fake Highlight")
...
#End If
    ...
    ''' <summary>
    ''' Extension method get the Right most bounds of this view
    ''' </summary>
    ''' <param name="aView"></param>
    ''' <returns></returns>
    <Extension> Public Function xMax(aView As DrawingView) As Double
        Return aView.Position.X + aView.Width / 2
    End Function

    ''' <summary>
    ''' Extension method get the left most bounds of this view
    ''' </summary>
    ''' <param name="aView"></param>
    ''' <returns></returns>
    <Extension> Public Function xMin(aView As DrawingView) As Double
        Return aView.Position.X - aView.Width / 2
    End Function

    ''' <summary>
    ''' Extension method get the bottom most bounds of this view
    ''' </summary>
    ''' <param name="aView"></param>
    ''' <returns></returns>
    <Extension> Public Function yMin(aView As DrawingView) As Double
        Return aView.Position.Y - aView.Height / 2
    End Function

    ''' <summary>
    ''' Extension method get the top most bounds of this view
    ''' </summary>
    ''' <param name="aView"></param>
    ''' <returns></returns>
    <Extension> Public Function ymax(aView As DrawingView) As Double
        Return aView.Position.Y + aView.Height / 2
    End Function

    ''' <summary>
    ''' Returns the point that is at the highest Y point on the sheet
    ''' </summary>
    ''' <param name="aCurve"></param>
    ''' <returns></returns>
    <Extension> Public Function YHighPoint(aCurve As DrawingCurve) As Point2d


        If aCurve.StartPoint.Y.Round > aCurve.EndPoint.Y.Round Then
            Return aCurve.StartPoint
        Else
            Return aCurve.EndPoint
        End If

    End Function

    ''' <summary>
    ''' Returns the point that is at the lowest Y point on the sheet
    ''' </summary>
    ''' <param name="aCurve"></param>
    ''' <returns></returns>
    <Extension> Public Function YLowPoint(aCurve As DrawingCurve) As Point2d
    ...
    ''' <summary>
    ''' Change the scale to fit view inside the box size defined by X and Y
    ''' </summary>
    ''' <param name="aView"></param>
    ''' <param name="X"></param>
    ''' <param name="Y"></param>
    <Extension> Public Sub rescaleView(aView As DrawingView, X As Double, Y As Double)
        Dim xOrg As Double = aView.Width / aView.Scale 'Unscaled width
        Dim yOrg As Double = aView.Height / aView.Scale 'Unscaled height
